...
if(!nombre.value)
{
	apprise("El nombre es requerido", {"animate":true}, function(){
		nombre.focus();
	});
	verificar = false;
}
...
{
	apprise("El nombre solo acpeta letras y espacios en blanco");
	nombre.focus();
	verificar = false;
}
...
{
	apprise("La edad solo acpeta 2 numeros");
	edad.focus();
	verificar = false;
}

else if(isNaN(edad.value)){
	apprise("La edad solo acepta numeros");
	edad.focus();
	verificar = false;
}
...
{
	apprise("La edad debe de oscilar entre los 18 y 60 años");
	edad.focus();
	verificar = false;
}
...
{
	apprise("Debes ingresar un correo");
	email.focus();
	verificar = false;
}
...
{
	apprise("El correo ingresado no es valido");
	email.focus();
	verificar = false;
}
...
{
	apprise("Se requiere un password");
	clave.focus();
	verificar = false;
}
...
// {
// 	apprise("La contraseña debe de tener minimo 4 caracteres alfanumericos y máximo 6");
// 	clave.focus();
// 	verificar = false;
// }
...
{
	apprise("Se requiere confirmar el password");
	clave2.focus();
	verificar = false;
}
...
{
	apprise("Las contraseñas no coinciden");
	clave.focus();
	verificar = false;
}
...
{
	apprise("Se necesita una región");
	region.focus();
	verificar = false;
}
else if(!archivo.value )
{
	apprise("Se necesita un archivo");
	archivo.focus();
	verificar = false;
}
...
{
	apprise("Proporciona tu You tube");
	youtube.focus();
	verificar = false;
}
...
{
	apprise("Ingresa algun comentario");
	comentarios.focus();
	verificar = false;
}
...
{
	apprise("Tu twitter debe tener minimo 2 caracteres y debe empezar con @");
	twitter.focus();
	verificar = false;
}
...
{
	apprise("El comentario no puede tener más de 255 caracteres");
	comentarios.focus();
	verificar = false;
}
...
{
	apprise("Debes aceptar los terminos y condiciones");
	condiciones.focus();
	verificar = false;
}
